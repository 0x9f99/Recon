...
EnumSubDomains(){
    ...
    rm -rf dns.tmp
    for i in `cat dns.target`;do host $i|grep -E -o "([0-9]{1,3}[\.]){3}[0-9]{1,3}";done |sort|uniq|grep -E -o "([0-9]{1,3}[\.]){3}"|uniq -c|tee ip.tmp
    cat ip.tmp | awk '{if ($1>=3) print $2"0/24"}' >ip.txt && rm -rf ip.tmp
}
